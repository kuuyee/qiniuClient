<template>
  <div class="layout-header">
    ...
    <template>
      ...
      <div
        v-if="isSupportUrlUpload"
        @mouseenter="toggleShow($event)"
        @mouseleave="toggleShow($event)"
      >
    ...
    <Dropdown @on-click="clickMore">
      <i-button class="button" type="text">
        <Icon type="md-more" size="24" />
      </i-button>
      <DropdownMenu slot="list">
        <DropdownItem name="7">
          <div @click="actionBtn(7)">{{`批量导出资源URL`}}</div>
        </DropdownItem>
        <DropdownItem name="6">
    ...
    <Input
      ...
      v-model="search"
      placeholder="搜索"
      :icon="search.length > 0 ? 'md-close-circle' : ''"
      @on-enter="actionBtn(2)"
      @on-click="clearSearch"
    />

    <!-- 同步对话框-->
    <Modal
      v-model="model_merge.show"
      title="请选择同步方式"
...
export default {
  ...
  mounted() {
    ...
    if (this.$storage.key === brand.upyun.key && !this.bucket.domain) {
      this.$Message.warning({
        content:
          "请先设置又拍云的加速域名.您可以在又拍云控制台查看您的加速域名",
        duration: 8
      });
    }
  },
  methods: {
    ...
    updateSupport() {
      this.isSupportUrlUpload =
        [brand.qiniu.key, brand.qingstor.key].indexOf(this.$storage.key) !== -1;
      this.isSupportDomain =
        [brand.qiniu.key, brand.tencent.key, brand.upyun.key].indexOf(
          this.$storage.key
        ) !== -1;
    },
    onChangeDomain(e) {
    ...
    syncFolder() {
      ...
      files.forEach((item, index) => {
        files[index].url = this.$parent.$refs["resource-grid"].getResourceUrl(
          item
        );
      });
